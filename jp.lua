...
local file_CreateDir                    = gFile.CreateDir
...
ultimate.accent = Color( 255, 255, 255, 255 )
...
ultimate.cfg.vars["SWB nospread mode"]          = 1
...
ultimate.cfg.vars["Rainbow"]                 = false
ultimate.cfg.colors["Watermark"]             = "255 255 255 255"
...
ultimate.cfg.vars["resolver_mode"] = 1
...
ultimate.cfg.vars["CStrafe prediction"] = false
ultimate.cfg.colors["CStrafe prediction"] = "0 255 0 255"
...
// Player visuals 
ultimate.cfg.colors["Lines"] = "255 255 255 255"
...
ultimate.cfg.colors["Box gradient 2"] = "255 0 255 255"
...
ultimate.cfg.vars["Indicators"] = true
ultimate.cfg.vars["GTA"] = false
ultimate.cfg.colors["GTA"] = "0 128 0 255"
...
if not file.Exists( "ultimate", "DATA" ) then
    file.CreateDir("ultimate")
end

if not file.Exists( "ultimate/default.txt", "DATA" ) then
    -- Сохраняем только vars, colors, binds, friends, ents
    local defaultConfig = {
        vars = ultimate.cfg.vars,
        colors = ultimate.cfg.colors,
        binds = ultimate.cfg.binds,
        friends = ultimate.cfg.friends,
        ents = ultimate.cfg.ents
    }
    file.Write( "ultimate/default.txt", util.TableToJSON( defaultConfig, false ), "DATA" )
end
...
function ultimate.SaveConfig()
    ...
    local tojs = util.TableToJSON( configToSave, false )

    if not file.Exists( "ultimate", "DATA" ) then
        file.CreateDir("ultimate")
    end
    
    file.Write( "ultimate/"..ultimate.cfg.vars["Config name"]..".txt", tojs, "DATA" )
...
function ultimate.LoadConfig()
    ...
    if not file.Exists( "ultimate/"..str..".txt", "DATA" ) then return end
    ...
    -- Загрузка vars
    if totbl.vars then
        for key, value in pairs( totbl.vars ) do
            ultimate.cfg.vars[ key ] = value
        end
    end

    -- Загрузка colors
    if totbl.colors then
        for key, value in pairs( totbl.colors ) do
            ultimate.cfg.colors[ key ] = value
        end
    end

    -- Загрузка binds
    if totbl.binds then
        for key, value in pairs( totbl.binds ) do
            ultimate.cfg.binds[ key ] = value
        end
    end

    -- Загрузка friends
    if totbl.friends then
        for key, value in pairs( totbl.friends ) do
            ultimate.cfg.friends[ key ] = value
        end
    end

    if ded and ded.SetInterpolation then ded.SetInterpolation( ultimate.cfg.vars["Disable interpolation"] ) end
    if ded and ded.SetSequenceInterpolation then ded.SetSequenceInterpolation( ultimate.cfg.vars["Disable Sequence interpolation"] ) end
    if ded and ded.EnableAnimFix then ded.EnableAnimFix( ultimate.cfg.vars["Update Client Anim fix"] ) end
    if ded and ded.EnableBoneFix then ded.EnableBoneFix( ultimate.cfg.vars["Bone fix"] ) end
end

function ultimate.CreateConfig()
    local name = ultimate.cfg.vars["Config name"]
    ...
    file.Write( "ultimate/"..name..".txt", tojs, "DATA" )
    ultimate.fillConfigTable()
    ultimate.initTab("Settings")
end
...
--sound.PlayURL ( "https://rus.hitmotop.com/get/music/20240307/Quairan_Mellstroy_-_CHTO_ZA_BIZNES_SUKA_ALORS_ON_DANSE_REMIX_upd_77545629.mp3", "noblock", function( s ) 
...
--end )
--https://cdn.discordapp.com/attachments/1261712219154944051/1264313954730246307/mellstroy.mp3
...
do
    ...
    function PANEL:Paint(w, h)
        ...
        if ultimate.cfg.vars["Rainbow"] then
            col = ultimate.accent
        end

        surface_SimpleText(8, 4, "ULTIMATE | Mellstroy Kick Edition", col) 
    end
...
do
    ...
    function PANEL:Think()
        ...
        if ultimate.ui and ultimate.ui.ColorWindow and IsValid( ultimate.ui.ColorWindow ) then
            return
        end
...
function ultimate.ui.ColorPicker( cfg, par, onChange )
    ...
    function b:DoClick()
        ...
        ultimate.ui.ColorWindow = vgui_Create( "UColorPanel", IsValid(ultimate.ui.SettingsPan) and ultimate.ui.SettingsPan or nil )
        ...
        function c.HSV.Knob:Paint( w, h )
            local col = b.Color or ultimate.Colors[255]
            surface_SetDrawColor( col.r or 255, col.g or 255, col.b or 255, col.a or 255 )
            surface_DrawRect( 0, 0, w, h )
...
function ultimate.ui.ComboBox( pan, str, cfg, choices )
    ...
    local current = ultimate.cfg.vars[cfg]
    if type(current) ~= "number" or current < 1 or current > #choices then
        dropdown:ChooseOptionID(1)
    else
        dropdown:ChooseOptionID(current)
    end
...
ultimate.spfuncs[30] = function()
   ...
   --ultimate.ui.CheckBox( ultimate.ui.SettingsPan, "Force seed", "Force seed" ) -- not work
    --ultimate.ui.CheckBox( ultimate.ui.SettingsPan, "Wait for seed", "Wait for seed" ) -- useless

    ultimate.ui.ComboBox( ultimate.ui.SettingsPan, "SWB Nospread", "SWB nospread mode", { "HVH", "RusElite" } )
...
function ultimate.tabs.Aimbot()
    ...
    local p = ultimate.itemPanel("Main",1,165):GetItemPanel()
    ...
    ultimate.ui.CheckBox( p, "Silent aim", "Silent aim", "Makes aim invisible on clientside." )
    ultimate.ui.CheckBox( p, "pSilent", "pSilent", "Context vector will be used to make aim completely invisible." )
    ultimate.ui.CheckBox( p, "Knife bot", "Knifebot","Automatically hit target with knife(support SWB, HL2, CS:GO", false, false, ultimate.spfuncs[3] )
    
    local p = ultimate.itemPanel("Legit",1,55):GetItemPanel()
    ...
    local p = ultimate.itemPanel( "Visualisation", 1, 190 ):GetItemPanel()

    ultimate.ui.CheckBox( p, "Show FOV", "Show FOV", false, false, true, ultimate.spfuncs[911] )
    ...
    ultimate.ui.ComboBox( p, "Marker Figure", "Aimbot marker mode", { "Square", "Circle" } )
    ultimate.ui.CheckBox( p, "GTA Marker", "GTA", false, false, true)
	ultimate.ui.CheckBox( p, "ТПОЗА(ENABLE)", "TPOSE" )

    local p = ultimate.itemPanel( "Accuracy", 2, 75 ):GetItemPanel()

    ultimate.ui.CheckBox( p, "Compensate recoil", "Norecoil", "Makes aim perfectly" )
    ultimate.ui.CheckBox( p, "Compensate spread", "Nospread", "Supported HL2, M9K, FAS2, CW2, SWB", false, false, ultimate.spfuncs[30] )

    local p = ultimate.itemPanel( "Prediction", 2, 75 ):GetItemPanel()

    ultimate.ui.CheckBox( p, "Projectile Aimbot", "Projectile aimbot" )
	ultimate.ui.CheckBox( p, "Engine Prediction", "Engine Prediction", "Can break aimbot" ) 
    ...
    local p = ultimate.itemPanel( "Hitbox selection", 3, 250 ):GetItemPanel()
...
function ultimate.tabs.Rage()
    local p = ultimate.itemPanel( "Angles", 1, 200 ):GetItemPanel()
    ...
    local p = ultimate.itemPanel( "Custom angles", 1, 150 ):GetItemPanel()
	...
	local p = ultimate.itemPanel( "Animation breaker", 1, 165 ):GetItemPanel()
    ...
    local p = ultimate.itemPanel( "Player adjustments", 3, 165 ):GetItemPanel()
    ...
    local p = ultimate.itemPanel( "Resolver", 3, 160 ):GetItemPanel()

    ultimate.ui.CheckBox( p,"Enable resolver", "Resolver" )
    ultimate.ui.ComboBox( p, "Resolver mode", "resolver_mode", { "Bruteforce 1", "Bruteforce 2", "Bruteforce 3" } )
    ...
    local p = ultimate.itemPanel( "Explosives", 3, 95 ):GetItemPanel()
...
ultimate.spfuncs[11] = function()
    ultimate.ui.SettingsPan:SetSize(250,165)
    
    ultimate.ui.CheckBox( ultimate.ui.SettingsPan, "Team color", "Box team color" )
    ultimate.ui.CheckBox( ultimate.ui.SettingsPan, "Gradient", "Box gradient", false, false, true )
    ultimate.ui.ComboBox( ultimate.ui.SettingsPan, "Style", "Box style", { "Default", "Corner", "3D" })
...
ultimate.spfuncs[14] = function()
    ultimate.ui.SettingsPan:SetSize(250,185)
...
function ultimate.tabs.Visuals()
    ...
    local p = ultimate.itemPanel("Player",1,495):GetItemPanel()
    ...
    ultimate.ui.CheckBox( p, "DarkRP Money", "DarkRP Money", false, false, false, false, false, function(p) local lbl, drop = ultimate.ui.ComboBox( p, "", "Money pos", {"Up","Down","Right","Left"} ) lbl:Remove() drop:Dock(RIGHT) drop:DockMargin(0,0,0,0) end )
    ...
    --ultimate.ui.CheckBox( p, "Fake pitch", "Fake pitch", false, false, false, false, false, function(p) local lbl, drop = ultimate.ui.ComboBox( p, "", "Fake pitch pos", {"Up","Down","Right","Left"} ) lbl:Remove() drop:Dock(RIGHT) drop:DockMargin(0,0,0,0) end )
    ...
    ultimate.ui.CheckBox( p, "Lines", "Lines", false, false, true, false, false, function(p) end )
    ...
    local p = ultimate.itemPanel("World",3,255 ):GetItemPanel()
    ...
    local p = ultimate.itemPanel( "Misc", 3, 145 ):GetItemPanel()
...
ultimate.spfuncs[26] = function()
    ...
    ultimate.ui.CheckBox( ultimate.ui.SettingsPan, "Line", "CStrafe prediction" )
end
...
function ultimate.tabs.Misc()
    ...
    local p = ultimate.itemPanel("Key spam",1,160):GetItemPanel()
    ...
    ultimate.ui.ComboBox( p, "Mode", "Chat mode", { "Quotes", "HvH", "Спермотоксикоз", "1", "Samoware","ChitDebila" })
    ...
    ultimate.ui.ComboBox( p, "Mode", "Killsay mode", { "Quotes", "HvH", "Спермотоксикоз", "1", "Samoware","ChitDebila" })


    ultimate.itemPanelB( "Net / Cvar", 3, 300, func )

    local p = ultimate.itemPanel("Memes",3,145):GetItemPanel()
...
function ultimate.tabs.Settings()
    local p = ultimate.itemPanel("Config",1,355):GetItemPanel()

    ultimate.ui.TextEntry( "Config name", "Config name", p, 64 )
    ultimate.ui.CheckBox( p,"Rainbow Text", "Rainbow", "Do text not rainbow" )
    ultimate.ui.CheckBox( p, "Watermark", "Watermark", false, false, true )
    ...
    local infoText = "ultimate v3.4: \n\nПравила использования:\nПросить чит - чмо,лох.\nПросить кфг- лох\nВсе в этом мире паста самовара"

    local infoLabel = vgui.Create("DLabel", p)
    infoLabel:SetText(infoText)
    infoLabel:SetFont("DermaDefault")
    infoLabel:SetTextColor(Color(105, 105, 105))
...
function ultimate.tabs.Entities()
    local entitylist = ents_GetAll()
    local classes = {}

    for i = 1, #entitylist do
        local gEntity = entitylist[ i ]

        local class = gEntity:GetClass()
        ...
        function pButton:Paint( w, h )
            if not IsValid(gEntity) then self:Remove() return end

            local class = gEntity:GetClass()

            surface_SetFont( "tbfont" )

            if ( ultimate.entityCache[ class ] ) then
        ...
        function pButton:OnMousePressed( mouseCode )
            ...
            if ( mouseCode == MOUSE_LEFT ) then
                ...
                if not ultimate.entityCache[ class ] then
                    ultimate.entityCache[ class ] = true
                else
                    ultimate.entityCache[ class ] = nil
                end
            end
        end
    end
...
function ultimate.CanShoot( cmd )
	...
	if ultimate.cfg.vars["Bullet time"] and ultimate.cfg.vars["Time"] != 1 and ultimate.activeWeapon:GetNextPrimaryFire() >= ded.GetServerTime(cmd)-ultimate.cfg.vars["Time"]  then
		return false
	end
...
function ultimate.CustomSpread.swb( cmd, ang )
    ...
    local mode = ultimate.cfg.vars["SWB nospread mode"] or 2
    ...
    if mode == 1 then
        ...
        if ultimate.activeWeapon.dt and ultimate.activeWeapon.AimSpread and ultimate.activeWeapon.dt.State == SWB_AIMING then
            ...
            if ultimate.activeWeapon.Owner and ultimate.activeWeapon.Owner.Expertise then
                ultimate.activeWeapon.BaseCone = ultimate.activeWeapon.BaseCone *
                    (1 - (ultimate.activeWeapon.Owner.Expertise["steadyme"] and ultimate.activeWeapon.Owner.Expertise["steadyme"].val or 0) * 0.0015)
            end
        else
            ...
            if ultimate.activeWeapon.Owner and ultimate.activeWeapon.Owner.Expertise then
                ultimate.activeWeapon.BaseCone = ultimate.activeWeapon.BaseCone *
                    (1 - (ultimate.activeWeapon.Owner.Expertise["wepprof"] and ultimate.activeWeapon.Owner.Expertise["wepprof"].val or 0) * 0.0015)
            end
        end

        if me:Crouching() then
            ultimate.activeWeapon.BaseCone =
                ultimate.activeWeapon.BaseCone *
                (ultimate.activeWeapon.dt and ultimate.activeWeapon.dt.State == SWB_AIMING and 0.9 or 0.75)
        end

        local unpredicted = false
        local updatetime = ultimate.activeWeapon.GetSpreadUpdateTime and ultimate.activeWeapon.GetSpreadUpdateTime(unpredicted) or 0
        local value = ultimate.activeWeapon.GetSpreadUpdateValue and ultimate.activeWeapon.GetSpreadUpdateValue(unpredicted) or 0

        if updatetime > 0 then
            value = math_max(0, math_min(ultimate.activeWeapon.MaxSpreadInc or 0,
                value - 0.1333 * (CurTime() - updatetime)))
        end

        local viewAffinity = ultimate.activeWeapon.GetViewAffinity and ultimate.activeWeapon.GetViewAffinity(unpredicted) or 0
        local affinityTime = ultimate.activeWeapon.GetViewAffinityTime and ultimate.activeWeapon.GetViewAffinityTime() or CurTime()
        local fireDelay = ultimate.activeWeapon.FireDelay or 0.2
        local value2 = math_max(0, math_min(2,
            viewAffinity -
            (ultimate.activeWeapon.ShotgunReload and 0.13 or 0.18) *
            (CurTime() - affinityTime) / fireDelay))

        ultimate.activeWeapon.CurCone = math_max(0, math_min((0.09 + (ultimate.activeWeapon.MaxSpreadInc or 0)),
            (ultimate.activeWeapon.BaseCone or 0) +
            value +
            ((vel / 10000) * (ultimate.activeWeapon.VelocitySensitivity or 0)) *
            (ultimate.activeWeapon.dt and ultimate.activeWeapon.dt.State == SWB_AIMING and (ultimate.activeWeapon.meMobilitySpreadMod or 1) or 1) +
            value2))
        ...
        if ultimate.activeWeapon.ClumpSpread and ultimate.activeWeapon.ClumpSpread > 0 then
            dir2 = dir1 + Angle(
                ...
                math_Rand(-1, 1),
                math_Rand(-1, 1)
            ) * ultimate.activeWeapon.ClumpSpread
        end
...
function ultimate.CustomSpread.arccw( cmd, ang )
    if not ultimate.activeWeapon then return ang end

    local spread = 0
    if ultimate.activeWeapon.GetCurrentSpread then
        spread = ultimate.activeWeapon:GetCurrentSpread() or 0
    elseif ultimate.activeWeapon.GetDispersion then
        spread = ultimate.activeWeapon:GetDispersion() or 0
    elseif ultimate.activeWeapon.CurCone then
        spread = ultimate.activeWeapon.CurCone or 0
    end

    if spread <= 0 then return ang end

    local spreadVec = Vector(spread, spread, 0)
    local spreadDir = ded.PredictSpread(cmd, ang, spreadVec)

    local compensated = ang + spreadDir:Angle()
...
ultimate.SpreadComps["arccw"]   = ultimate.CustomSpread.arccw
...
function ultimate.PredictPosition(ply, ticks)
    if not IsValid(ply) then return ply:GetPos() end
    
    local origin = ply:GetPos()
    local velocity = ply:GetVelocity()
    local prediction = origin + (velocity * TickInterval * ticks)
    

    prediction.z = prediction.z + (0.5 * 600 * (TickInterval * ticks) ^ 2)
    
    return prediction
end

function ultimate.GetBestBone(ply, eyepos, mode)
    local bones = ultimate.GetBones(ply)
    if not bones or #bones == 0 then return bones[1] end
    
    if mode == 1 then 
        local best, bestDist = nil, math_huge
        for i = 1, #bones do
            local dist = (bones[i] - eyepos):LengthSqr()
            if dist < bestDist then
                best, bestDist = bones[i], dist
            end
        end
        return best
    elseif mode == 2 then 
        local priority = {1, 2, 3, 4, 5, 6, 7, 0} // head > chest > stomach > arms > legs > generic
        for _, hg in ipairs(priority) do
            for i = 1, #bones do
                local set = ply:GetHitboxSet()
                local group = ply:GetHitBoxHitGroup(i - 1, set)
                if group == hg then return bones[i] end
            end
        end
    elseif mode == 3 then 
        local headBones, bodyBones = {}, {}
        for i = 1, #bones do
            local set = ply:GetHitboxSet()
            local group = ply:GetHitBoxHitGroup(i - 1, set)
            if group == 1 then table_insert(headBones, bones[i])
            elseif group == 2 or group == 3 then table_insert(bodyBones, bones[i])
            end
        end
        if #headBones > 0 and math_random() < 0.7 then
            return headBones[math_random(1, #headBones)]
        elseif #bodyBones > 0 then
            return bodyBones[math_random(1, #bodyBones)]
        end
        return bones[1]
    end
...
function ultimate.CalculateRecoil(cmd, weapon)
    if not IsValid(weapon) then return 0, 0 end

    local punch = cmd:GetViewAngles() - me:EyeAngles()

    local recoilX = punch.p * 0.5
    local recoilY = punch.y * 0.3

    return recoilX, recoilY
end

function ultimate.HumanizeAngle(targetAng, currentAng, fov)

    local fovRatio = math_Clamp(fov / 30, 0, 1)
    local baseSmooth = 5
    local dynamicSmooth = baseSmooth + (1 - fovRatio) * 15
    

    local jitter = Vector(math_Rand(-0.3, 0.3), math_Rand(-0.3, 0.3), 0)
    
    local ret = LerpAngle(FrameTime() * dynamicSmooth, currentAng, targetAng)
    ret.p = ret.p + jitter.x
    ret.y = ret.y + jitter.y
...
function ultimate.Aim(cmd)
    ...
    if ultimate.cfg.vars["Prediction enabled"] and IsValid(ply) then
        local ticks = ultimate.cfg.vars["Prediction ticks"]
        local predictedPos = ultimate.PredictPosition(ply, ticks)
        local eyePos = me:EyePos()
        

        local predictionOffset = predictedPos - bone
        if predictionOffset:Length() > 0 then
            finalAngle = (bone + predictionOffset * 0.5 - eyePos):Angle()
            finalAngle:Normalize()
        end
    end


    if ultimate.cfg.vars["Bone priority"] and ultimate.cfg.vars["Bone priority"] > 0 then
        local eyepos = me:EyePos()
        local bestBone = ultimate.GetBestBone(ply, eyepos, ultimate.cfg.vars["Bone priority"])
        if bestBone then
            finalAngle = (bestBone - eyepos):Angle()
            finalAngle:Normalize()
        end
    end
    ...
    // FOV-based Smoothing с Humanization
    if ultimate.cfg.vars["Aimbot smoothing"] then
        local va = cmd:GetViewAngles()
        va.r = 0
        
        local fovDelta = finalAngle - va
        fovDelta:Normalize()
        local currentFov = math_sqrt(fovDelta.x * fovDelta.x + fovDelta.y * fovDelta.y)
        
        if ultimate.cfg.vars["FOV-based smoothing"] then
            finalAngle = ultimate.HumanizeAngle(finalAngle, va, currentFov)
        else
            local rat = ultimate.cfg.vars["Smoothing"] * 100
            finalAngle = LerpAngle(FrameTime() * rat, va, finalAngle)
        end
        
        // Humanization - случайная задержка перед началом аима
        if ultimate.cfg.vars["Humanization"] then
            local reactionDelay = ultimate.cfg.vars["Reaction delay"] / 1000
            local humanizeDelay = ultimate.cfg.vars["Humanize delay"] / 1000
            local totalDelay = reactionDelay + math_Rand(0, humanizeDelay)
            
            // Применяем задержку через таймер (упрощённо)
            if CurTime() % 1 < totalDelay then
                finalAngle = va
            end
        end
    end

    // Randomize Aimpoint для легитности
    if ultimate.cfg.vars["Randomize aimpoint"] and IsValid(ply) then
        local spread = ultimate.cfg.vars["Aimpoint spread"]
        local randomOffset = Vector(
            math_Rand(-spread, spread),
            math_Rand(-spread, spread),
            math_Rand(-spread * 0.5, spread * 0.5)
        )
        finalAngle = (bone + randomOffset - me:EyePos()):Angle()
        finalAngle:Normalize()
    end
...
function ultimate.PredictMovement( viewangles, dir, angle, save_path )

	local pm
    if save_path then ultimate.cstrafe_path = {} end
	...
	for i = 1, pticks do
		...
		origin = pm.HitPos
        if save_path then ultimate.cstrafe_path[i] = origin end
...
function ultimate.CircleStrafe( cmd )
	...
	while ( ultimate.cstrafe_dir < 2 ) do
		...
		while ( true ) do
			...
			if ( ultimate.PredictMovement( cmd:GetViewAngles(), ultimate.cstrafe_dir, angle, true ) ) then
...
function ultimate.watermark()
    ...
    local col = string_ToColor( ultimate.cfg.colors["Watermark"] )
    if ultimate.cfg.vars["Rainbow"] then
        col = ultimate.accent
    end

    local text = (ultimate.cfg.vars["Custom Cheat"] or "ULTIMATE Mellstroy Kick Edition") .. " | " .. LocalPlayer():Name() .. " | gm: " .. engine.ActiveGamemode() .. " | latency:" .. LocalPlayer():Ping() .. " | tick:" .. math.Round(1 / engine.TickInterval() - 1) ..""
    surface_SetFont("tbfont")
    ...
    surface_SetMaterial(Material("gui/center_gradient"))
    surface_DrawTexturedRect(7, 6, pedic-4, 3)
    draw.SimpleText(text, "tbfont", 12, 10, col )
end
    ...
    for i = 1, #plys do
        ...
        if not isVisible then
            if ultimate.cfg.vars["Lines"] then
                ...
                local distance = myEyePos:Distance(targetPos)
                local maxDistance = tonumber(ultimate.cfg.vars["Max Distance"]) or 1000000
                ...
                if distance <= maxDistance then
                    ...
                    local baseX, baseY = scrw / 2, scrh
                    local isFriend = ultimate.cfg.friends[v:SteamID64()]
                    local col = isFriend and Color(0, 255, 0) or string_ToColor( ultimate.cfg.colors["Lines"] )

                    local endX, endY = targetScreen.x, targetScreen.y
                    if isVisible then
                        endX = MinX + (MaxX - MinX) / 2
                        endY = MaxY
                    else
                        endX = targetScreen.x
                        endY = targetScreen.y
                    ...
                    surface_SetDrawColor(0, 0, 0, lineAlpha)
                    surface_DrawLine(baseX - 1, baseY, endX - 1, endY)
                    surface_DrawLine(baseX + 1, baseY, endX + 1, endY)
                    surface_DrawLine(baseX, baseY - 1, endX, endY - 1)
                    surface_DrawLine(baseX, baseY + 1, endX, endY + 1)

                    surface_SetDrawColor(col.r, col.g, col.b, lineAlpha)
                    surface_DrawLine(baseX, baseY, endX, endY)
                end
            end
            continue
        end
        ...
        if ultimate.cfg.vars["Box esp"] then
            local isFriend = ultimate.cfg.friends[v:SteamID64()]
            local boxColor = isFriend and Color(0, 255, 0) or (ultimate.cfg.vars["Box team color"] and teamcolor or color_box)
            ...
            if ultimate.cfg.vars["Box style"] == 1 then
                ...
                if ultimate.cfg.vars["Box gradient"] then
                    surface_SimpleTexturedRect( MinX, MinY, XLen, 1, color_box_g, ultimate.Materials["Gradient right"] )
                    surface_SimpleTexturedRect( MinX, MaxY-1, XLen, 1, color_box_g, ultimate.Materials["Gradient right"] )
                    surface_SimpleTexturedRect( MinX, MinY, 1, YLen, color_box_g, ultimate.Materials["Gradient"] )
                    surface_SimpleTexturedRect( MaxX-1, MinY, 1, YLen, color_box_g, ultimate.Materials["Gradient"] )
                end
            elseif ultimate.cfg.vars["Box style"] == 2 then
                ...
                if ultimate.cfg.vars["Box gradient"] then
                    surface_SimpleTexturedRect( MinX, MinY, wlen, 1, color_box_g, ultimate.Materials["Gradient right"] )
                    surface_SimpleTexturedRect( MinX, MinY, 1, hlen, color_box_g, ultimate.Materials["Gradient"] )
                    surface_SimpleTexturedRect( MaxX - wlen + 1, MinY, wlen, 1, color_box_g, ultimate.Materials["Gradient right"] )
                    surface_SimpleTexturedRect( MaxX, MinY, 1, hlen, color_box_g, ultimate.Materials["Gradient"] )
                    surface_SimpleTexturedRect( MinX, MaxY - 1, wlen, 1, color_box_g, ultimate.Materials["Gradient right"] )
                    surface_SimpleTexturedRect( MinX, MaxY - hlen - 1, 1, hlen, color_box_g, ultimate.Materials["Gradient"] )
                    surface_SimpleTexturedRect( MaxX - wlen + 1, MaxY - 1, wlen, 1, color_box_g, ultimate.Materials["Gradient right"] )
                    surface_SimpleTexturedRect( MaxX, MaxY - hlen - 1, 1, hlen, color_box_g, ultimate.Materials["Gradient"] )
                end
            elseif ultimate.cfg.vars["Box style"] == 3 then
                ...
                if ultimate.cfg.vars["Box gradient"] then
                    surface_SimpleTexturedRect( MinX, MinY + hlen - 1, 1, hlen, color_box_g, ultimate.Materials["Gradient"] )
                    surface_SimpleTexturedRect( MaxX, MinY + hlen - 1, 1, hlen, color_box_g, ultimate.Materials["Gradient"] )
                end
            elseif ultimate.cfg.vars["Box style"] == 4 then
                ...
                for i = 1, 3 do
                    ...
                    surface_SetDrawColor( i == 2 and boxColor or ultimate.Colors[0] )
            ...
            elseif ultimate.cfg.vars["Box style"] == 5 then
                cam_Start3D()
                render_DrawWireframeBox(v:GetPos(), v:GetAngles(), v:OBBMins(), v:OBBMaxs(), boxColor, true)
                cam_End3D()
            end
        end
        ...
        if ultimate.cfg.vars["Name"] then 
            ...
            local x = (poses[pos] or 0) - (ultimate.getTextX(tw,pos) or 0)
            local y = ultimate.getTextY(MaxY,MinY,th,pos,ttbl[pos] or 0)
            if x and y then
                surface_SetTextPos(x, y)
                surface_DrawText(name)
            end

            ttbl[pos] = (ttbl[pos] or 0) + 0.8
        end
        ...
        if health > 0 then
            if ultimate.cfg.vars["Health"] then 
                ...
                local h_col = Color( 255 - ( 255 * ( health / maxhealth ) ), 255 * ( health / maxhealth ), 0 )

                surface_SetTextColor( h_col )
                local x = (poses[pos] or 0) - (ultimate.getTextX(tw,pos) or 0)
                local y = ultimate.getTextY(MaxY,MinY,th,pos,ttbl[pos] or 0)
                if x and y then
                    surface_SetTextPos(x, y)
        ...
        if ultimate.cfg.vars["Fake pitch"] and v.fakepitch then
            local name = "FAKE PITCH"
            local pos = ultimate.cfg.vars["Fake pitch pos"]
            local tw, th = surface_GetTextSize(name)
        ...
        -- Надпись "Friend" для друзей
        local isFriend = ultimate.cfg.friends[v:SteamID64()]
        if isFriend then
            local name = "Friend"
            local pos = ultimate.cfg.vars["Name pos"] -- Используем ту же позицию что и у имени
            local tw, th = surface_GetTextSize(name)
...
if ultimate.cfg.vars["Lines"] then  -- LINII
            ...
            for _, v in pairs(ents_GetAll()) do
                if v:IsPlayer() and v ~= me and v:Alive() then
                    local targetPos = v:LocalToWorld(v:OBBCenter())
                    local distance = me:EyePos():Distance(targetPos)

                    if distance <= maxDistance then
                        local alpha = math_Clamp((maxDistance - distance) / maxDistance, 0, 1)
                        local lineAlpha = alpha * 255

                        local targetScreen = targetPos:ToScreen()
                        local eyeScreen = me:EyePos():ToScreen()

                        if targetScreen.visible and eyeScreen.visible then
                            surface_SetDrawColor(255, 255, 255, lineAlpha)
                            surface_DrawLine(eyeScreen.x, eyeScreen.y, targetScreen.x, targetScreen.y)
                        end
                    end
                end
            end
        ...
        */
        ...
        if ultimate.cfg.vars["Fov limit"] and ultimate.cfg.vars["Show FOV"] then
            ...
            if ultimate.cfg.vars["Show FOV Fill"] then
                ...
                if ultimate.cfg.vars["FOV Rainbow"] then
                    local function HSVToColor(h, s, v, a)
                        ...
                        return Color(math_floor((r + m) * 255), math_floor((g + m) * 255), math_floor((b + m) * 255), a or 255)
                    end
                    ...
                    surface_SetDrawColor(col.r, col.g, col.b, fillAlpha)
                    for i = 0, segments - 1 do
                        local angle1 = (i / segments) * 2 * math.pi
                        local angle2 = ((i + 1) / segments) * 2 * math.pi
                        local x1 = cx + math_cos(angle1) * radius
            ...
            if ultimate.cfg.vars["FOV Rainbow"] then
                local function HSVToColor(h, s, v, a)
                    ...
                    return Color(math_floor((r + m) * 255), math_floor((g + m) * 255), math_floor((b + m) * 255), 255)
                end
                local rainbow_offset = CurTime() * 0.2
                for i = 0, segments - 1 do
                    local angle1 = (i / segments) * 2 * math.pi
                ...
                local function DrawCircle(x, y, r, c)
                    surface_SetDrawColor(c.r, c.g, c.b, c.a)
                    for i = 0, segments - 1 do
                        local angle1 = (i / segments) * 2 * math.pi
                        local angle2 = ((i + 1) / segments) * 2 * math.pi
                        local x1 = x + math_cos(angle1) * r
                        local y1 = y + math_sin(angle1) * r
                        local x2 = x + math_cos(angle2) * r
                        local y2 = y + math_sin(angle2) * r
                        surface_DrawLine(x1, y1, x2, y2)
                    end
                end
                
                DrawCircle(cx, cy, radius + 1, Color(0, 0, 0, 128))
        ...
        if ultimate.cfg.vars[ "Spectator list" ] then
            ...
            for i = 1, #plys do
                ...
                table.insert(spectators, v:Name() .. " spectating " .. tostring( ultimate.playerCache[ v ].ObserverTarget ))
            end
            ...
            surface.SetDrawColor(139, 0, 0, 255)
            surface.DrawRect(x, y - 1, w, 1)


            surface.SetDrawColor(173, 255, 47, 255) 
            surface.DrawRect(x, y, w, h)
            
            surface.SetFont("veranda")
            local tw, th = surface.GetTextSize("Spectator list")
            

            surface.SetTextColor(0, 0, 0, 255)
            surface.SetTextPos(x + (w - tw) / 2 + 1, y + (h - th) / 2 + 1)
            surface.DrawText("Spectator list")


            surface.SetTextColor(139, 0, 0, 255)
            surface.SetTextPos(x + (w - tw) / 2, y + (h - th) / 2)
            surface.DrawText("Spectator list")
            ...
            if #spectators > 0 then
                surface.SetFont("veranda")
                for _, text in ipairs(spectators) do
        ...
        if ultimate.cfg.vars["GTA"] and ultimate.targetVector then
            local pos = ultimate.targetVector:ToScreen()
            local color = string_ToColor( ultimate.cfg.colors["GTA"] )
...
function ultimate.CalcMainActivity(ply, velocity)
    if ultimate.cfg.vars[ "TPOSE" ] then
        return -1, -1
    end
end
...
function ultimate.PrePlayerDraw(ply, falgs)
	if ply != me then
        ...
        if ultimate.cfg.vars["Disable taunts"] then
            for i = 0, 13 do
...
function ultimate.PostDrawTranslucentRenderables()
    ...
    if ( ultimate.cfg.vars["Circle strafe"] and ultimate.cfg.vars["CStrafe prediction"] and ultimate.cstrafe_path and ultimate.IsKeyDown(ultimate.cfg.binds["Circle strafe"]) ) then
        local col = string_ToColor( ultimate.cfg.colors["CStrafe prediction"] )
        render_SetMaterial( Material( "vgui/white" ) )
        
        local last_pos
        for i = 1, #ultimate.cstrafe_path do
            local pos = ultimate.cstrafe_path[i]
            if last_pos then
                render_DrawBeam( last_pos, pos, 1, 0, 1, col )
            end
            last_pos = pos
        end
    end
...
{
        [1] = { // Trashtalk
            "*DEAD* G̷o̷r̷o̷x̷: 1 - Gorox",  
            "Чекушка окончило свое выступление и вступило в команду DEAD - odysse99",
            "Что такое виртуозно? - Gorox",
        },
        [2] = { // Лучшие 22-23
            "1 нищий упал",
            "$$$ кешбек по зубам $$$",
...
ultimate.bruteYaw = {-180, -135, -90, -45, 0, 45, 90, 135, 180}
ultimate.bruteforce = {-180, -89, 0, 90, 180}
ultimate.zalupa = {-45, 45}
    ...
    function ultimate.hFrameStageNotify( stage )
        ...
        elseif stage == FRAME_RENDER_START then
            ...
            for i = 1, #plys do
                ...
                if ultimate.cfg.vars["Resolver"] then
                    local resolverMode = ultimate.cfg.vars["resolver_mode"] or 1
                    local angs = Angle()
                    local eyeAngles = v:EyeAngles()
                    
                    if not v.aimshots then v.aimshots = 0 end
                    
                    if resolverMode == 1 then 
                        local bruteIndex = (v.aimshots % #ultimate.bruteYaw) + 1
                        angs.y = ultimate.bruteYaw[bruteIndex] + eyeAngles.y
                    elseif resolverMode == 2 then 
                        local bruteforceIndex = (v.aimshots % #ultimate.bruteforce) + 1
                        angs.y = ultimate.bruteforce[bruteforceIndex] + eyeAngles.y
                    elseif resolverMode == 3 then 
                        local zalupaIndex = (v.aimshots % #ultimate.zalupa) + 1
                        angs.y = ultimate.zalupa[zalupaIndex] + eyeAngles.y
                    end
                    ...
                    local bodyYaw = math.NormalizeAngle(angs.y - eyeAngles.y)
                    v:SetPoseParameter("body_yaw", bodyYaw)
                    v:SetPoseParameter("aim_yaw", bodyYaw * 0.5)
                ...
                if ultimate.cfg.vars["Taunt resolver"] then
                    for i = 0, 13 do
                        if v:IsValidLayer(i) then
                            local seqname = v:GetSequenceName(v:GetLayerSequence(i))
                            if seqname:StartWith("taunt_") or seqname:StartWith("act_") or seqname:StartWith("gesture_") then
                                local weight = v:GetLayerWeight(i)
                                if weight > 0.1 then
                                    local tauntAng = Angle(0, eyeAngles.y + 180, 0)
                                    v:SetRenderAngles(tauntAng)
                                    v:SetPoseParameter("body_yaw", 180)
                                    v:SetPoseParameter("aim_yaw", 90)
                                    v:InvalidateBoneCache()
                                    v:SetupBones()
                                    break
                                end
...
ultimate.AddHook( "Ungrabbable2D", function() ultimate.watermark() ultimate.DrawESP() ultimate.DrawSomeShit() end )   
